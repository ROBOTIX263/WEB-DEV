    ...
    if (element !== null) {
      ...
      if (
        ...
        if (!element.style.borderImage)
            element.style.borderImage = `radial-gradient(${offset * 3}px ${offset * 3}px at ${bx}px ${by}px ,rgba(255,255,255,0.7),rgba(255,255,255,0.1),transparent ) 9 / 2px / 0px stretch `;
        return [...acc, element];
      }
    }
    return acc;
